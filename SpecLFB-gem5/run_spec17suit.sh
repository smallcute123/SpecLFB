...
int_rate=(perlbench_r gcc_r  omnetpp_r xalancbmk_r x264_r deepsjeng_r leela_r exchange2_r xz_r)
frate=(bwaves_r namd_r parest_r wrf_r povray_r blender_r cam4_r imagick_r nab_r fotonik3d_r)
int_rate1000=(mcf_r)
 
for i in "${!int_rate[@]}"  
do 
cd $GEM5_DIR
source ./run_spec17.sh "${int_rate[$i]}" m5out 
done

for i in "${!frate[@]}"  
do 
cd $GEM5_DIR 
source ./run_spec17.sh "${frate[$i]}" m5out 
done
