  ...
  with HasBoomFrontendParameters // TODO: Don't add this trait
{
  ...
  //**********************************
  // construct 555all of the modules
  ...
  for (w <- 0 until coreWidth) {
    ...
    if (coreWidth == 1) {
      dis_uops(w).rob_idx := rob.io.rob_tail_idx
        if(DISTRACE_PRINTF){
             printf("DT %x %x\n",dis_uops(w).debug_pc,dis_uops(w).rob_idx)
        }
    } else {
...
//   "CSRFile is being written to spuriously.")
  ...
  //io.lsu.rob_unsafe_masked :=rob.io.rob_unsafe_masked
  //[speclfb]add
  io.lsu.unsafe_masked :=rob.io.unsafe_masked
...
//[speclfb]Passing unsafe signals from lsu unit to rob
  rob.io.lsu_unsafe_mem := io.lsu.unsafe_mem
  ...
  if (usingRoCC) {
    ...
    //exe_units.rocc_unit.io.rocc.rob_unsafe_masked :=rob.io.rob_unsafe_masked
    //[speclfb]add
    exe_units.rocc_unit.io.rocc.unsafe_masked :=rob.io.unsafe_masked
