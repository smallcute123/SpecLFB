...
//  val rob_pnr_idx  = Output(UInt(robAddrSz.W))
  ...
  //[speclfb]
  val lsu_unsafe_mem   = Input(Vec(memWidth, Valid(UInt(robAddrSz.W))))
  ...
  )(implicit p: Parameters) extends BoomModule
{
 ...
 // val rob_pnr_idx  = if (coreWidth == 1) rob_pnr  else Cat(rob_pnr , rob_pnr_lsb)
  ...
  //[speclfb] initial rob unsafe mask , the ROB unsafe mask has the same number of bits as the number of rows in ROB
val unsafe_masked = WireInit(VecInit(Seq.fill(numRobRows){false.B}))
  ...
  for (w <- 0 until coreWidth) {
    ...
    //[speclfb]add for unresolved branch and memory order(SSB)
    val rob_unsafe_branch = Reg(Vec(numRobRows, Bool()))
    val rob_unsafe_lsu = Reg(Vec(numRobRows, Bool()))
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    for (clr <- io.lsu_clr_unsafe) {
      when (clr.valid && MatchBank(GetBankIdx(clr.bits))) {
        ...
        rob_unsafe(cidx) := false.B//修改
      }
    }
    //[speclfb]For unresolved store-load data dependency in SSB
    for (unsafe <- io.lsu_unsafe_mem) {
      when (unsafe.valid && MatchBank(GetBankIdx(unsafe.bits))) {
        val cidx = GetRowIdx(unsafe.bits)
        rob_unsafe_lsu(cidx) := true.B
        if(UNSAFE_TRACE){
        printf("rob lsu %x %x\n",rob_uop(cidx).debug_pc, rob_unsafe_lsu(cidx).asUInt);}           
    ...
    when (io.lxcpt.valid && MatchBank(GetBankIdx(io.lxcpt.bits.uop.rob_idx))) {
      ...
      if(ROBTRACE_PRINTF){
        printf("RT %x %x\n",io.lxcpt.bits.uop.debug_pc,io.lxcpt.bits.cause)
      }
    ...
    for (i <- 0 until numRobRows) {
      ...
      //[speclfb]For unresolved branch
      rob_unsafe_branch(GetRowIdx(rob_uop(i).rob_idx)):= Mux(br_mask === 0.U, 
                                      false.B,true.B)
    if(brMask_PRINTF){
        printf(" %x %x %x\n",rob_uop(i).debug_pc,rob_uop(i).br_mask,rob_unsafe_branch(GetRowIdx(rob_uop(i).rob_idx)));}                       
    ...
    for (i <- 0 until numRobRows) {
     ...
     //[speclfb]update rob unsafe mask
      unsafe_masked(i) := rob_val(i) && (rob_unsafe(i) || rob_exception(i)||rob_unsafe_branch(i)||rob_unsafe_lsu(i))
      // if(SPECULATIVE_TRACE){
      //   when(rob_uop(i).is_br||rob_uop(i).is_jal||rob_uop(i).is_jalr){
      //     printf(" %x 2\n",rob_uop(i).debug_pc)
      //   }
      //   when(rob_exception(i)){
      //     printf(" %x 3\n",rob_uop(i).debug_pc)
      //   } 
      //   when(rob_unsafe(i)){
      //  printf(" %x 1\n",rob_uop(i).debug_pc)
 ...
 // io.rob_unsafe_masked := rob_unsafe_masked
 io.unsafe_masked:=unsafe_masked
  io.empty        := empty
