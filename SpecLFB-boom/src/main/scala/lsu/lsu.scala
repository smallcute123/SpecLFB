...
//[speclfb]:for musls in unresolved memory access order
  val unsafe_mem = Output(Bool())
...
io.dmem.unsafe_masked := io.core.unsafe_masked
  ...
  for (w <- 0 until memWidth) {
    ...
    //可能产生 ordering failures,或者处于错误的分支预测下
    // val fired_load_wakeup    = widthMap(w => RegNext(will_fire_load_wakeup  (w) && !IsKilledByBranch(io.core.brupdate, ldq_wakeup_e.bits.uop)))
    io.core.clr_unsafe(w).bits  := RegNext(lcam_uop(w).rob_idx)

    //[speclfb]Determine whether the current load has an address conflict with the uncommitted store instruction
  //   for (i <- 0 until numStqEntries) {
  //   val s_addr = stq(i).bits.addr.bits
  //   val s_uop  = stq(i).bits.uop
   
  //   when((ldst_addr_matches(w)(i) && IsOlder(s_uop.stq_idx, stq_commit_head, stq_head))|| !ldst_addr_matches(w)(i)){

  //      io.core.unsafe_mem(w).valid := false.B
  //      io.core.unsafe_mem(w).bits  := RegNext(lcam_uop(w).rob_idx)

  //      if(UNSAFE_TRACE){
  //       printf("safe load %x %x %x %x %x %x\n",s_uop.debug_pc,ldst_addr_matches(w)(i).asUInt,s_uop.stq_idx,stq_commit_head,stq_head,io.core.unsafe_mem(w).bits)
  //     }
  // }
  io.core.unsafe_mem := (0 until numStqEntries).map{ i => stq(i).valid && (!stq(i).bits.addr.valid ) }.reduce(_||_) =/= 0.U
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    if (MEMTRACE_PRINTF) {
      when (commit_store || commit_load) {
        ...
        printf("MT %x %x %x %x %x %x %x %x %x %x %x\n",
          io.core.tsc_reg, uop.debug_pc,uop.rob_idx<<log2Ceil(coreWidth),uop.unsafe,uop.inst,uop.uopc, uop.mem_cmd, uop.mem_size, addr, stdata, wbdata)
      }
    }
