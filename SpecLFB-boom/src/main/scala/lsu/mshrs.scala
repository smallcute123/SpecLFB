...
{
  // miss info缓存缺失信息
  val tag_match = Bool()//dcache的input   mshrs.io.req(w).bits.tag_match   := s2_tag_match(w) tag是否和地址匹配
  val old_meta  = new L1Metadata//存储tag以及缓存一致性信息
  ...
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
    ...
    //val rob_unsafe_masked = Input(Vec(numRobRows << log2Ceil(coreWidth),Bool()))
    //[speclfb]add
    val unsafe_masked =Input(Vec(numRobRows,Bool()))
    ...
    val mem_acquire = Decoupled(new TLBundleA(edge.bundle))//TLBundle是Tilelink定义各个channel的输入输出接口，这里定义的是channel A：传送一个请求，访问指定的地址范围或对数据进行缓存操作。

    val mem_grant   = Flipped(Decoupled(new TLBundleD(edge.bundle)))//因为这是属于link对于acquire的回复，channel D：向最初的请求者传送一个数据回复响应或是应答消息。
    ...
    // To inform the prefetcher when we are commiting the fetch of this line 与预取器有关
  ...
  val grow_param = new_coh.onAccess(req.uop.mem_cmd)._2//map(_._n)表示任意元组tuple对象,后面的数字n表示取第几个数.(n>=1的整数)返回的是下一步的消息参数
  ...
  val (_, _, refill_done, refill_address_inc) = edge.addr_inc(io.mem_grant)//返回了每个beat需要的地址,以及当前beat传输是否完成
  ...
  val rpq = Module(new BranchKillableQueue(new BoomDCacheReqInternal, cfg.nRPQ, u => u.uses_ldq, false))// replay queue entries，
  /*会创建两个队列，enq是入队队列，deq是出队队列，且两个队列的in-out是相反的*/
  rpq.io.brupdate := io.brupdate
  rpq.io.flush  := io.exception
  assert(!(state === s_invalid && !rpq.io.empty))

  rpq.io.enq.valid := ((io.req_pri_val && io.req_pri_rdy) || (io.req_sec_val && io.req_sec_rdy)) && !isPrefetch(io.req.uop.mem_cmd)//由发送请求cache方决定，且enq.valid only high if not killed by branch
  ...
  //val can_commit_line = Reg(Bool())//用于loads安全检查阶段决定后面是否rpq队列的loads都已提交
  ...
  def handle_pri_req(old_state: UInt): UInt = {//对初始req之后的状态
    ...
    when (io.req.tag_match) {
      val (is_hit, _, coh_on_hit) = old_coh.onAccess(io.req.uop.mem_cmd)//mem_cmd在uop中的定义：sync primitives/cache flushes
      /*is_hit bool变量 表示是否命中，true表示cache hit且这个时候的状态是dirty，false代表当前指令的操作对对cache节点无权限

      coh_on_hit就是得到的updatedMetadata
      */
      ...
      when (is_hit) { // set dirty bit
        ...
        new_coh     := old_coh
        new_state   := s_refill_req//这个时候cache节点处于branch或者nothing的状态需要进入s_refill_req状态发出重填请求
      }
    } .otherwise { // refill and writeback if necessary
      new_coh     := ClientMetadata.onReset//Client node的缓存行信息重置
  ...
  when (state === s_invalid) {
    ...
    when (io.req_pri_val && io.req_pri_rdy) {//valid是输入信号由request即cache产生
      state := handle_pri_req(state)
    }
  } .elsewhen (state === s_refill_req) {//请求一个新的缓存行
  ...
  } .elsewhen (state === s_refill_resp) {//将请求到的新缓存行放入line-fill buffer
    when (edge.hasData(io.mem_grant.bits)) {//hasData函数判断mem_grant返回的消息是否含有数据
      io.mem_grant.ready      := io.lb_write.ready//ready由consumer驱动数据接受者
      io.lb_write.valid       := io.mem_grant.valid//valid由producer驱动数据生产者
    ...
    when (refill_done) {//refill_done代表当前beat的传输是否完成
      grantack.valid := edge.isRequest(io.mem_grant.bits)//channelD是否在执行请求
      grantack.bits := edge.GrantAck(io.mem_grant.bits)
      //state := Mux(grant_had_data, s_drain_rpq_loads, s_drain_rpq)//应答消息中是否有数据有的话就到s_drain_rpq_loads的状态
    ...
    //                !io.rob_unsafe_masked(rpq.io.deq.bits.uop.rob_idx<<log2Ceil(coreWidth)) &&
    ...
    when (rpq.io.deq.fire && drain_load) {
    //[Speclfb]safe check, unsafe load instructions cannot enter the `commit_line` state to refill.
    if(SPECLFB){
    when (!io.unsafe_masked(rpq.io.deq.bits.uop.rob_idx >> log2Ceil(coreWidth))){
       commit_line   := true.B
       if(UNSAFE_TRACE){
        printf("%x \n",rpq.io.deq.bits.uop.debug_pc)
       }
      ...
      .elsewhen (rpq.io.empty && !commit_line)
    {
      when (!rpq.io.enq.fire) {
        ...
        finish_to_prefetch := enablePrefetching.B//是否允许预取
      }
  ...
  } .elsewhen (state === s_meta_clear) {//cacheline进行clear
  ...
  with HasL1HellaCacheParameters
{
  val io = IO(new Bundle {
   ...
   // val rob_unsafe_masked = Input(Vec(numRobRows << log2Ceil(coreWidth),Bool()))
   //[speclfb]add
    val unsafe_masked =Input(Vec(numRobRows,Bool()))//added
  ...
  val tag_match   = widthMap(w => Mux1H(idx_matches(w), tag_matches(w)))//idx_matches的序列第w个的值为y，则返回的是tag_matches的序列第y个的值
  ...
  val mshrs = (0 until cfg.nMSHRs) map { i =>
    ...
    //mshr.io.rob_unsafe_masked := io.rob_unsafe_masked
    //[speclfb]add
   mshr.io.unsafe_masked := io.unsafe_masked
