...
int main(void){
    ...
    for (uint64_t offset = 0; offset < SECRET_SZ; ++offset){
        ...
        printf("\033[0;32;32mvaddr[%010p]:offset(%lu): want(%c) : (hit-times,ASICC,char) = 1.(%lu, %d, %c) 2.(%lu, %d, %c)\033[m\n", 
        (uint8_t*)(secretString + offset),offset,secretString[offset], hitArray[0], output[0], output[0], 
                                                           hitArray[1], output[1], output[1]); 
        else 
        printf("\033[0;32;31mvaddr[%010p]:offset(%lu): want(%c) : (hit-times,ASICC,char) = 1.(%lu, %d, %c) 2.(%lu, %d, %c)\033[m\n", 
        (uint8_t*)(secretString + offset), offset,secretString[offset], hitArray[0], output[0], output[0], 
                                                           hitArray[1], output[1], output[1]);  
    }
}
